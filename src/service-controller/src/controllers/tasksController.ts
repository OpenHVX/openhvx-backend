...
exports.enqueueTask = async (req, res) => {
    try {
        ...
        // 1) if the action consumes quotas, place a hold right away
        ...
        if (deltas) {
            ...
            try {
                ...
                if (e?.code === 'QUOTA_EXCEEDED' || e?.status === 409) {
                    return send(res, e, req); // immediate rejection when quotas are insufficient
                }
                throw e;
            }
        }

        // 2) Create the task and publish it to the broker
